...
export function LoginForm({
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => {
      ...
      // Invalidate queries that might depend on user login state
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      await mutation.mutateAsync(value); // Trigger mutation with form values
    },
  });
      ...
      <div className="flex flex-col gap-6">
        ...
        <form
          onSubmit={(e) => {
            e.preventDefault();
            e.stopPropagation();
            form.handleSubmit(); // Use form.handleSubmit from useForm
          }}
          ...
          <form.Field
            ...
            children={({ state, handleChange, handleBlur }) => (
              <div className="grid gap-2">
                ...
                <Input
                  id="identity"
                  type="text" // Fixed: Use 'text' for email or username
            ...
            children={({ state, handleChange, handleBlur }) => (
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  checked={state.value}
                  onCheckedChange={() => handleChange(!state.value)}
                  onBlur={handleBlur}
                />
                <label
                  htmlFor="remember"
